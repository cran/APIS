...
#' @param verbose (default : TRUE) Display the process of the function on the console.
...
APIS <- function(off.genotype, sire.genotype, dam.genotype, error = 0,
  ...
  if (ncol(off.genotype) == ncol(sire.genotype) & ncol(off.genotype) == ncol(dam.genotype) & ncol(sire.genotype) == ncol(dam.genotype)) {
    if (verbose) { cat("genotype matrices : OK \n") }
  } else {
  ...
  if ((0 <= exclusion.threshold) && (exclusion.threshold <= ncol(off.genotype))) {
    if (verbose) { cat("exclusion threshold : OK \n") }
  } else {
  ...
  if ((0 <= error) && (error <= 100) && (is.numeric(error))) {
    if (verbose) { cat("assignment error rate : OK \n") }
  } else {
    stop("The assignment error rate limit is NEGATIVE")
  }

  # Check if all offspring markers have at least one genotype
  offspring.markerGeno <- apply(off.genotype, 2, function(X) {
    if (length(X[which(X == "NA/NA")]) == length(X)) {
      return(TRUE)
    } else {
      return(FALSE)
    }})

  marker_nonGeno <- which(offspring.markerGeno == TRUE)
  if (length(marker_nonGeno) == 0) {
    if (verbose) { cat("All the markers are genotyped \n") }
  } else {
    id_marker <- colnames(off.genotype)[marker_nonGeno]
    off.genotype  <- off.genotype[, -marker_nonGeno]
    sire.genotype <- sire.genotype[, -marker_nonGeno]
    dam.genotype  <- dam.genotype[, -marker_nonGeno]
  ...
  if (verbose) { cat("The assignment power of your marker set is ", P2, "% \n", sep = "") }

  if (P >= 0.99) {
    if (verbose) { cat("Theoretical assignment power : OK \n") }
  } else {
  ...
  assignResult 	<- assignmentFortran(offspring = off.genotype,
                                     ...
                                     nb.cores = nb.cores,
                                     verbose = verbose)
  apisResult 		<- setThreshold(ped.log = assignResult$log.mendel, ped.exclu = assignResult$exclu,
                               nb.mrk = assignResult$nb.mrk, error = error, verbose = verbose)
  ...
  if (verbose) {
    ...
    cat('--------------------------------------', sep = '\n')
    cat('Theoretical assignment power of the marker set : ', P2, "% \n", sep = "")
    cat('Assignment error rate accepted : ', error, '\n')
    assignmentRate <- length(pedigree$sire[which(!is.na(pedigree$sire))]) / nrow(pedigree)
    AR <- substr(as.character(100 * assignmentRate), 1, 6)
    cat('Assignment rate : ', AR, '% \n', sep = "")
  }
...
assignmentFortran <- function(offspring, sire, dam, thresh = ncol(offspring),
  ...
  if (verbose) { cat('Estimation of allele frequencies \n') }
  ...
  if (verbose) { cat('Recoding datasets \n') }
  ...
  if (verbose) { cat("Number of cores used :", nb.cores, "\n") }
  ...
  if (verbose) {
    ...
    recode.off <- foreach(i = 1:iterations, .combine = f_recode_off_verbose()) %dopar% {
      tmp <- as.numeric(as.vector(sapply(offspring[i,, drop = FALSE], recodeFortran, list.mrk = variant.corres)))
    }
  ...
  if (verbose) {
    f_recode_sire_verbose <- function(){
      pb <- txtProgressBar(min = 0, max = nrow(sire), char = '><> ', style = 3)
      count <- 1
      function(...) {
        count <<- count + length(list(...)) - 1
        setTxtProgressBar(pb,count)
    ...
    recode.sire <- foreach(i = 1:nrow(sire), .combine = f_recode_sire_verbose()) %dopar% {
      tmp <- as.numeric(as.vector(sapply(sire[i,, drop = FALSE], recodeFortran, list.mrk = variant.corres)))
    }
  ...
  if (verbose) {
    f_recode_dam_verbose <- function(){
      pb <- txtProgressBar(min = 0, max = nrow(dam), char = '><> ', style = 3)
      count <- 1
      function(...) {
        count <<- count + length(list(...)) - 1
        setTxtProgressBar(pb,count)
    ...
    recode.dam <- foreach(i = 1:nrow(dam), .combine = f_recode_dam_verbose()) %dopar% {
      tmp <- as.numeric(as.vector(sapply(dam[i,, drop = FALSE], recodeFortran, list.mrk = variant.corres)))
    }
  ...
  add.freqMatrix <- matrix(data = c(add.freq, rep(0, times = length(add.freq) * (nrow(Freq) - 1))),
                           ncol = length(add.freq), nrow = nrow(Freq), byrow = TRUE)
  ...
  if (verbose) {
    f_assign_verbose <- function() {
      pb <- txtProgressBar(min = 1, max = iterations, char = "><(((*> ", style = 3)
      count <- 1
      function(...) {
        count <<- count + length(list(...)) - 1
        setTxtProgressBar(pb,count)
    ...
    A <- foreach(off = 1:iterations, .combine = f_assign_verbose()) %dopar% { # For each offspring
      ...
      res2 <- res[order(res[,4], res[,3], decreasing = TRUE),]
      ...
      #Order by mismatches
      res2 <- res[order(res[,3], -res[,4], decreasing = FALSE),]

      out.exclu <- unlist(c(offspring.name[off], genotyped.mrk[off], res2[1,1:3], res2[2,1:3], res2[3,1:3]))

      return(c(out.log, out.exclu))
    }
  } else {
    A <- foreach(off = 1:iterations, .combine = rbind) %dopar% { # For each offspring
  ...
  ped.exclu <- as.data.frame(A[,17:ncol(A)])
...
#' @param ped.log log.like from assignment function
#' @param ped.exclu log.exclu from assignment function
...
#' @importFrom "stats" "median" "quantile"
#' @export
setThreshold <- function(ped.log, ped.exclu, nb.mrk, error = NULL, verbose = TRUE) {
  ...
  if (verbose) { cat('Estimated number of offspring with at least one missing parent : between',2*N1_1min,'and',2*N1_1, "\n") }
  ...
  if ((2*N1_1)<=round(error*nrow(ped.log))) {
    ...
    s.delta23 <- sort(ped.log$delta_Pmendel23, decreasing = TRUE)

    thresh.mendel <- quantile(s.delta23[1:(nrow(ped.log) - 2*N1_1min)], probs = (1-error), type = 5, na.rm = TRUE)
    if (verbose) { cat('Threshold for delta :', thresh.mendel, '\n') }
...
allFreq <- function(genotype) {
  ...
  for (i in c(1:ncol(genotype))) {
    tmp <- strsplit(genotype[,i], split = '/', fixed = TRUE)
...
#' @importFrom "stats" "median" "quantile"
#' @export
personalThreshold <- function(APIS.result, method, threshold = NULL, verbose = TRUE) {
  P <- NULL
  ...
  if (verbose) {
    ...
    cat('\n')
    assignmentRate <- length(pedigree$sire[which(is.na(pedigree$sire) == FALSE)]) / nrow(pedigree)
    AR <- substr(as.character(100 * assignmentRate), 1, 6)
...
selectParents <- function(off.genotype, parent.genotype, parent.sex, n.Parent) {
  ...
  for (p in output$parent) {
    ...
    # Write the result
    output[i, 3] <- sum(parent_probability, na.rm = TRUE)
    i <- i + 1
  }
