...
vignette: >
  %\VignetteIndexEntry{Introduction to APIS}
  %\VignetteEngine{knitr::rmarkdown}b 
  \usepackage[utf8]{inputenc}
---

# Description
This package includes all the functions to assign with APIS.
...
In this package, you will find all the functions to perform parentage assign.
...
Use the function as below, with default parameters for exclusion threshold and preselection of parents to maximize the reliability.
...
<br>
```{r full_data, echo = F, message = F, warning = F, fig.width=8, fig.height=4}
result <- APIS(off.genotype = APIS_offspring,
               sire.genotype = APIS_sire,
               dam.genotype = APIS_dam,
               error = 0.05,
...
<br>
```{r degraded_data, echo = F, message = F, warning = F, results = "hide", fig.width=8, fig.height=4}
...
APIS function can handle 2 other parameters :
  ...
  * verbose : Display information while the function is running. Give more details about the theoretical assignment rate, estimated number of offspring with missing parents and display progress bar.
